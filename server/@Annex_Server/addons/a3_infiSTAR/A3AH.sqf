...
/* *********************************www.infiSTAR.de********************************* */
diag_log '<TacticalGaming.net> VERSION: 01-Dec-2016 19-28-44 TacticalGaming.net AHAT (v0244a)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='nelson16852@gmail.com';
INFISTARVERSION='01-Dec-2016 19-28-44 - v0244a';
if((toLower UPDATEEMAIL) in ['','infiSTAR@ok.de','admin@infistar.de'])exitWith{
	diag_log '<TacticalGaming.net> Please go to update.infistar.de and download the latest version of infiSTAR!';
	diag_log format['<TacticalGaming.net> The current version is licensed to %1 and not your email address.',UPDATEEMAIL];
	diag_log '<TacticalGaming.net> infiSTAR will NOT START if it is licensed to that email address!';
};
diag_log format['<TacticalGaming.net> %1 - STARTING',time];
_t = time;
diag_log format['<TacticalGaming.net> %1 - TESTING IF serverCommandPassword IS SET PROPERLY',_t];
_return = _serverCommandPassword serverCommand format ['#kick %1',0];
if(!_return)exitWith
{
	diag_log format['<TacticalGaming.net> %1 - serverCommandPassword NOT SET!    nelson16852@gmail.com - 01-Dec-2016 19-28-44 - v0244a - %2 - %3',_t,serverName,productVersion];
	diag_log format['<TacticalGaming.net> %1 - serverCommandPassword   in infiSTAR_config.sqf is %2',_t,_serverCommandPassword];
	diag_log format['<TacticalGaming.net> %1 - serverCommandPassword   is defined in your servers config.cfg',_t];
	diag_log format['<TacticalGaming.net> %1 - serverCommandPassword   if your config.cfg does not have serverCommandPassword yet, simply add it in a new line: serverCommandPassword = "changeme";',_t];
	diag_log format['<TacticalGaming.net> %1 - serverCommandPassword   has to be set it in infiSTAR_config.sqf where it says _serverCommandPassword = "changeme";',_t];
	diag_log format['<TacticalGaming.net> %1 - infiSTAR will not START if passwords are not set properly!',_t];
};
FN_GET_SERVERPW = compileFinal (str _serverCommandPassword);
diag_log format['<TacticalGaming.net> %1 - serverCommandPassword IS FINE',_t];
if!(isClass (missionconfigfile >> 'infiSTAR_AdminMenu'))exitWith
{
	diag_log format['<TacticalGaming.net> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [nelson16852@gmail.com - 01-Dec-2016 19-28-44 - v0244a - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'config\call.fsm';
_test = [0,{}] execFSM 'config\call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<TacticalGaming.net> %1 - call.fsm missing in your MPmission!    nelson16852@gmail.com - 01-Dec-2016 19-28-44 - v0244a - %2 - %3',_t,serverName,productVersion];
	for '_i' from 0 to 30 do
	{
		diag_log _log;
		diag_log format['<TacticalGaming.net> %1 - infiSTAR will not START if files are not in place!',_t];
	};
};
...
FNC_A3_LOG_RAND_VAR = compileFinal "
	'ARMA_LOG' callExtension format['A3_RANDOMVAR_LOG:%1 (v0244a)',_this];
	diag_log ('<TacticalGaming.net>RandomVarLog| '+_this+' (v0244a)');
";
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['q','A','V','q','D','S','G','Y','S','o','j','I','T','F','P','x','w','t','y','r','H','a','k','y','R','R','C','p','S','h'];
	...
	_gen
};diag_log format['<TacticalGaming.net> _fnc_RandomGen: %1',_fnc_RandomGen];
...
_FNC_AH2074_KICKLOG = call _fnc_RandomGen;format['_FNC_AH2074_KICKLOG: %1',_FNC_AH2074_KICKLOG] call FNC_A3_LOG_RAND_VAR;
_FNC_AH2074_KICKLOGSPAWN = call _fnc_RandomGen;format['_FNC_AH2074_KICKLOGSPAWN: %1',_FNC_AH2074_KICKLOGSPAWN] call FNC_A3_LOG_RAND_VAR;
...
_AH2074KICKLOG = call _fnc_RandomGen;format['_AH2074KICKLOG: %1',_AH2074KICKLOG] call FNC_A3_LOG_RAND_VAR;
...
_AH2074_KICKLOG = call _fnc_RandomGen;format['_AH2074_KICKLOG: %1',_AH2074_KICKLOG] call FNC_A3_LOG_RAND_VAR;
...
diag_log format['<TacticalGaming.net> %1 - loading AntiHack..',time];
...
FNC_A3_HACKLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_HACKLOG:%1 [FPS: %2|THREADS: %3] (v0244a)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<TacticalGaming.net>HL| '+_this+' (v0244a)');
";
FNC_A3_SURVEILLANCELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_SURVEILLANCELOG:%1 [FPS: %2|THREADS: %3] (v0244a)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<TacticalGaming.net>SL| '+_this+' (v0244a)');
";
FNC_A3_ADMINLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_ADMINLOG:%1 (v0244a)',_this];
	diag_log ('<TacticalGaming.net>AdminLog| '+_this+' (v0244a)');
";
FNC_A3_CONNECTLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_CONNECTLOG:%1 (v0244a)',_this];
	diag_log ('<TacticalGaming.net>ConnectLog| '+_this+' (v0244a)');
";
FNC_A3_TOKENLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_TOKENLOG:%1 (v0244a)',_this];
	diag_log ('<TacticalGaming.net>TokenLog| '+_this+' (v0244a)');
";
FNC_A3_RESOLVESTEAMNAMELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_RESOLVESTEAMNAMELOG:%1 (v0244a)',_this];
	diag_log ('<TacticalGaming.net>ResolveSteamName| '+_this+' (v0244a)');
...
fnc_call_ARMALOAD = compileFinal "
	...
	if(_option == 0)exitWith
	{
		...
		if(_url in ARMALOAD_urlARRAY)exitWith{diag_log '<TacticalGaming.net> terminated double ARMA_LOAD call';};
		...
		if(count _input == 3)then
		{
			...
			_code = {
				...
				_log = format['<TacticalGaming.net> %1(%2) SteamName is: %3',_name,_uid,_steamname];
...
_A3AHstring = "
	...
	_server_setToken={
		...
		if(isNil'_input')exitWith{diag_log '<TacticalGaming.net> server_setToken: _input is NIL';};
		...
		if(_inputUID isEqualTo '')exitWith{diag_log '<TacticalGaming.net> server_setToken: _inputUID is EMPTY';};
		...
		catch
		{
			...
			if(isNull _inputOBJECT)exitWith{diag_log format['<TacticalGaming.net> server_setToken: (%1) _inputOBJECT is NULL',_inputUID];};
			[_token,(owner _inputOBJECT),(name _inputOBJECT),_inputUID,_inputOBJECT] call "+_fnc_pvc_token+";
			diag_log format['<TacticalGaming.net> server_setToken: SENT TOKEN [%1] TO %2(%3)',_token,name _inputOBJECT,_inputUID];
		};
	};
	...
	'"+_RequestToken+"' addPublicVariableEventhandler {
		...
		_input = _this select 1;
		if(isNil '_input')exitWith{diag_log '<TacticalGaming.net> RequestToken: _input is Nil';};
		
		_clientUID = _input select 1;
		if(isNil '_clientUID')exitWith{diag_log '<TacticalGaming.net> RequestToken: _clientUID is Nil';};
		if!(typeName _clientUID isEqualTo 'STRING')exitWith{diag_log format['<TacticalGaming.net> RequestToken: typeName _clientUID != OBJECT -> %1',typeName _clientUID];};
		if(_clientUID isEqualTo '')exitWith{diag_log '<TacticalGaming.net> RequestToken: resolving _playerObj with UID not possible - no UID found';};
		
		_playerObj = _input select 0;
		if(isNil '_playerObj')exitWith{diag_log '<TacticalGaming.net> RequestToken: _playerObj is Nil';};
		if!(typeName _playerObj isEqualTo 'OBJECT')exitWith{diag_log format['<TacticalGaming.net> RequestToken: typeName _playerObj != OBJECT -> %1',typeName _playerObj];};
		if(isNull _playerObj)then
		{
			diag_log format['<TacticalGaming.net> RequestToken: resolving _playerObj of UID %1 via allPlayers',_clientUID];
			{
		...
		if(isNull _playerObj)exitWith{diag_log format['<TacticalGaming.net> RequestToken: %1(%2) _playerObj is NULL-OBJ',_playerObj,_clientUID];};
		
		_clientID = (owner _playerObj);
		if(isNil '_clientID')exitWith{diag_log format['<TacticalGaming.net> RequestToken: %1(%2) _clientID is Nil',name _playerObj,_clientUID];};
		
		_clientUIDcheck = getPlayerUID _playerObj;
		if(isNil '_clientUIDcheck')exitWith{diag_log format['<TacticalGaming.net> RequestToken: %1(%2) _clientUIDcheck is Nil',name _playerObj,_clientUID];};
		if!(_clientUID isEqualTo _clientUIDcheck)exitWith{diag_log format['<TacticalGaming.net> RequestToken: %1(%2) _clientUID != _clientUIDcheck  %2/%3',name _playerObj,_clientUID,_clientUIDcheck];};
		
		diag_log format['<TacticalGaming.net> RequestToken: %1(%2) Requested a Token!   _clientID: %3',name _playerObj,_clientUID,_clientID];
		if(isNil '"+_BigTokenArray+"')then{"+_BigTokenArray+" = [];};
		if(_clientUID in "+_BigTokenArray+")then
		{
			_token = "+_BigTokenArray+" select (("+_BigTokenArray+" find _clientUID)+1);
			if(isNil '_token')exitWith{diag_log format['<TacticalGaming.net> RequestToken: %1(%2) _token is Nil',name _playerObj,_clientUID];};
			[_token,_clientID,(name _playerObj),_clientUID,_playerObj] call "+_fnc_pvc_token+";
			diag_log format['<TacticalGaming.net> RequestToken: %1(%2) TOKEN FOUND! SENT %3 TO OBJECT WITH NETID %4',name _playerObj,_clientUID,_token,netId _playerObj];
		}
		else
		{
	...
	FNC_INFISERVERKICK = {
		...
		diag_log format['<TacticalGaming.net>KickLog| %1(%2)   (v0244a)',_name,_uid];
	};
	...
	FNC_A3_infiSTARBAN = {
		...
		if(_reason == '')then
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) (v0244a)',_name,_uid];
			diag_log format['<TacticalGaming.net>BanLog| %1(%2)   (v0244a)',_name,_uid];
		}
		else
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) %3 (v0244a)',_name,_uid,_reason];
			diag_log format['<TacticalGaming.net>BanLog| %1(%2) %3 (v0244a)',_name,_uid,_reason];
		};
...
diag_log format['<TacticalGaming.net> %1 - Thread MAIN: starting!',time];
...
"; }; _A3AHstring = _A3AHstring + "
	if({((toLower _name) find _x != -1)} count ['thirtysix','le hippo'] > 0)exitWith{
		...
		[_name,_uid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AH2074_KICKLOG+";
	};
...
fnc_infiSTAR_PlayerLog = compileFinal ([_fnc_infiSTAR_PlayerLog] call fnc_CompilableString);
	...
	diag_log format['<TacticalGaming.net> %1 - Thread MAIN: loading...!',time];
	diag_log format['<TacticalGaming.net> %1 - Thread MAIN: compiling and/or sending none-threaded code!',time];
				...
				if(!isNull _x)then
				{
					if(alive _x)then
					{
						if(name _x == _name)then
						{
							...
							diag_log format['<TacticalGaming.net> CLIENT NO UID - SERVER RESOLVED: %1(%2).. used playableUnits',_name,_puid];
						};
					};
				};
			} forEach playableUnits;
							...
							"; if(_UVC)then{ _A3AHstring = _A3AHstring + "
								if((!("+str _UVW+") && {_vehstype in "+str _ForbiddenVehicles+"}) || (("+str _UVW+") && {!(_vehstype in "+str _VehicleWhiteList+")}))then
								{
									...
									if(_crew isEqualTo [])then
									{
										...
										['SERVER','SERVER','HLOG',toArray(_log)] call "+_FNC_AH2074_KICKLOG+";
									...
									else
									{
										...
										_log = format['BadVehicle (S-UVC+LVC): %1',_vehstype];
										{
											_name = name _x;
											_puid = getPlayerUID _x;
											[_name,_puid,'HLOG',toArray(_log)] call "+_FNC_AH2074_KICKLOG+";
										} forEach _crew;
										_veh call fnc_deleteObject;
									};
								};
						...
						if(!isNull _veh)then
						{
							if((_plrObj == _veh) && (_vehstype != _vehptype))then
							{
								...
								if!(_veh in _vehicles)then
								{
									...
									[_name,_puid,'SLOG',toArray(_log)] call "+_FNC_AH2074_KICKLOG+";
								};
							};
						};
					};
	...
	"+_FNC_AH2074_KICKLOG+" = compileFinal ([_FNC_AH2074_KICKLOG] call fnc_CompilableString);
	...
	_FNC_PVAH_AdminReq = {
		...
		if(isNil '_clientNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is Nil (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _clientNetID != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq clientNetID wrong typeName: %1   %2 (v0244a)',typeName _clientNetID,_clientNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_clientNetID == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is empty (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		_ObjFromNetID = objectFromNetId _clientNetID;
		if(isNil '_ObjFromNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Nil (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _ObjFromNetID != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq ObjFromNetID wrong typeName: %1   %2 (v0244a)',typeName _ObjFromNetID,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _ObjFromNetID)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq ObjFromNetID is Null (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_array')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq is Nil (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _array != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq wrong typeName: %1   %2 (v0244a)',typeName _array,_array];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_option')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq option is Nil (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _option != 'SCALAR')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq option wrong typeName: %1   %2 (v0244a)',typeName _option,_option];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _playerObj != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj wrong typeName: %1   %2 (v0244a)',typeName _playerObj,_playerObj];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Null (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if!(_ObjFromNetID isEqualTo _playerObj)then
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj != _ObjFromNetID - %1/2  (v0244a)',_playerObj,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		...
		if(!isPlayer _playerObj)then
		{
			if(_option == 1234)then
			{
				...
				if(isNil '_puid')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + 'AdminReq puid is Nil (v0244a)';
					_log call FNC_A3_SURVEILLANCELOG;
				};
				if(typeName _puid != 'STRING')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['AdminReq puid wrong typeName: %1   %2 (v0244a)',typeName _puid,_puid];
					_log call FNC_A3_SURVEILLANCELOG;
				};
				
				_mytime = call fnc_getserverTime;
				_log = format['AdminReq playerObj is not a player   %1(%2 - %3) - trying to resolve player via allPlayers (v0244a)',name _playerObj,getPlayerUID _playerObj,_puid];
				_log = _mytime + _log;
				_log call FNC_A3_SURVEILLANCELOG;
				if(_puid == '')exitWith{};
				{
		...
		if(!isPlayer _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = format['AdminReq playerObj is not a player   %1(%2) (v0244a)',name _playerObj,getPlayerUID _playerObj];
			_log = _mytime + _log;
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is Nil (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq tokenreceived wrong typeName: %1   %2 (v0244a)',typeName _tokenreceived,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_tokenreceived == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is empty (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_clientUID isEqualTo '')then
		{
			...
			if(_foundtokenid > 0)then
			{
				...
				if(!isNil '_puidfound')then
				{
					...
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['%1(%2) | AdminReq - SERVER RESOLVED UID! (v0244a)',_clientName,_clientUID];
					_log call FNC_A3_SURVEILLANCELOG;
				};
			};
		};
		...
		if(!_result)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - BadToken %3 (v0244a)',_clientName,_clientUID,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		_fnc_NoUID = {
			...
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - !!!!!!ADMIN-LOGIN-ERROR!!!!!! (v0244a)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		call {
		...
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - Attempted to Use the AdminMenu (v0244a)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_option == 1234)exitWith
		{
			...
			if(_puid == _clientUID)then
			{
				if!(_clientUID in _locDevs)then{
					diag_log format['<TacticalGaming.net> ******ADMIN-LOGIN******: %1(%2) (v0244a)',_clientName,_clientUID];
				};
				"; if(_enableIngameLogs)then{ _A3AHstring = _A3AHstring + "
				...
				[
					'',
					{
						...
						diag_log format['<TacticalGaming.net> OPEN_ADMIN_MENU_KEY: %1',OPEN_ADMIN_MENU_KEY];
						...
						"; if(_LogAdminActions)then{ _A3AHstring = _A3AHstring + "
							if(isNil 'fnc_adminLog')then{if(_puid in "+str _devs+")then{fnc_adminLog = compileFinal '';}else{fnc_adminLog = compileFinal '[profileName,getPlayerUID player,''ALOG'',toArray _this] call "+_AH2074KICKLOG+";';};};
						"; }else{ _A3AHstring = _A3AHstring + "
	...
	_FNC_AH2074_KICKLOGSPAWN =
	{
		...
		if(isNil '_input')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'FNC_AH2074_KICKLOGSPAWN _this is Nil (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		diag_log format['<TacticalGaming.net FNC_AH2074_KICKLOGSPAWN> %1',_this];
		
		_tokenreceived = _this select 0;
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_tokenreceived _this is Nil (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_tokenreceived wrong type %1 (v0244a)',typeName _tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_arraysent')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_arraysent _this is Nil (v0244a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _arraysent != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_arraysent wrong type %1 (v0244a)',typeName _arraysent];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _netId != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_netId wrong type %1 (v0244a)',typeName _netId];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		diag_log format['<TacticalGaming.net FNC_AH2074_KICKLOGSPAWN> %1(%2) Token %3 - objByNetID %4',_name,_puid,_tokenreceived,_objectFromNetId];
		...
		else
		{
			if(_objectUID isEqualTo _puid)then
			{
				...
				{
					diag_log format['<TacticalGaming.net FNC_AH2074_KICKLOGSPAWN> %1(%2) PlayerObjectName is not equal to PlayerName!',_name,_puid,_objectName];
					_name = _objectName;
				};
			};
		};
		...
		if(!_result)exitWith
		{
			...
			diag_log format['<TacticalGaming.net FNC_AH2074_KICKLOGSPAWN> %1',_log];
			[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AH2074_KICKLOG+";
		};
		
		
		_arraysent set [0,_objectName];
		_arraysent set [1,_puid];
		_arraysent spawn "+_FNC_AH2074_KICKLOG+";
	};
	"+_FNC_AH2074_KICKLOGSPAWN+" = compileFinal ([_FNC_AH2074_KICKLOGSPAWN] call fnc_CompilableString);
	...
	addMissionEventHandler['HandleDisconnect',{
		...
		if(alive _plyr)then
		{
			...
			if(_inCombat == 1)then
			{
				...
				'ARMA_LOG' callExtension format['A3_COMBATLOG:%1 (v0244a)',_log];
				diag_log ('<TacticalGaming.net>CombatLog| '+_log+' (v0244a)');
...
_AH_MAIN_BLOCK = {
	...
	if(isNil'"+_AH2074KICKLOG+"')exitWith{
	...
	if!(_puid isEqualTo _clientUID)exitWith
	{
		...
		[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_AH2074KICKLOG+";
		(findDisplay 46)closeDisplay 0;
	};
	...
	if!(_puid in _admins)then
	{
		...
		if(_add_Id > -1)then
		{
			...
			if(_announce)then
			{
				...
				[profileName,_puid,'HLOG_SKICK',toArray(_log)] call "+_AH2074KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
		};

		{
			if(!isNil _x)then
			{
				...
				[profileName,_puid,'SLOG_SKICK',toArray(_log)] call "+_AH2074KICKLOG+";
				(findDisplay 46)closeDisplay 0;
			};
		} forEach [
		...
		_cfgPatches = 'true' configClasses (configFile >> 'CfgPatches');
		{
			...
			if(_lconfigName in _badclassesban)then
			{
				...
				[_name,_puid,'BAN',toArray(_log)] call "+_AH2074KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
			if(_lconfigName in _badclasseskick)then
			{
				_log = format['Bad Addon found in CfgPatches: %1',_configName];
				[_name,_puid,'HLOG_SKICK',toArray(_log)] call "+_AH2074KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
		} forEach _cfgPatches;
	...
	[_name,_puid,_admins] spawn {
		...
		"; if((_MOD == 'Epoch')&&(_AWL))then{ _A3AHstring = _A3AHstring + "
			...
			_fadeOut = {
				_faded = true;
				TitleText ['!!!\nDO NOT LOOK THROUGH WALLS\n!!!\n\n\n\nAntiHack by TacticalGaming.net','BLACK FADED'];
				if(!isNull findDisplay 602)then
				{
					...
					if(time > _lastCall)then
					{
						...
						[_name,_puid,'HLOG',toArray(_log)] call "+_AH2074KICKLOG+";
					};
				};
			};
			_dis_head = 0.20;
		...
		while{1==1}do
		{
						...
						if(_shape isEqualTo 'POLYLINE')then
						{
							...
							if!(_puid isEqualTo '')then
							{
								...
								if(_puid isEqualTo _varuid)then
								{
									...
									systemChat ('<TacticalGaming.net> '+_log);
									if(_need_hlog)then
									{
										[_name,_varuid,'HLOG',toArray(_log)] call "+_AH2074KICKLOG+";
									};
									_need_hlog = false;
								};
							};
			...
			if(_combatTime <= 30)then
			{
				...
				{
					_inCombat = 1;
					[_name,_puid,'COMBATCHECK',_inCombat] call "+_AH2074KICKLOG+";
				};
			}
			else
			{
			...
			if(!isNull _display49)then
			{
				...
				(_display49 displayCtrl 120) ctrlSetText 'TacticalGaming.net AntiHack & AdminsTools - [Author: infiSTAR, Contact: infiSTAR23@gmail.com]';
				"; }else{ _A3AHstring = _A3AHstring + "
				(_display49 displayCtrl 115025) ctrlSetText 'AntiHack & AdminsTools';
				(_display49 displayCtrl 115035) ctrlSetText 'by TacticalGaming.net';
				"; }; _A3AHstring = _A3AHstring + "
			};
				...
				if(_txt != '')then{
					...
					if(_puid in _admins)then
					{
						if(_txt == '!admin')exitWith
						{
							...
							[_name,_puid,'AC',toArray(_txt)] call "+_AH2074KICKLOG+";
							(findDisplay 46)closeDisplay 0;
						};
					}
					else
					...
					"; if(_VDN)then{ _A3AHstring = _A3AHstring + "
						if(_txt in ['!vote day','/vote day','vote day','!day','/day'])then{
							waitUntil {(findDisplay 24) closeDisplay 0;isNull findDisplay 24};
							[_name,_puid,'VOTE',toArray 'DAY'] call "+_AH2074KICKLOG+";
						};
						if(_txt in ['!vote night','/vote night','vote night','!night','/night'])then{
							waitUntil {(findDisplay 24) closeDisplay 0;isNull findDisplay 24};
							[_name,_puid,'VOTE',toArray 'NIGHT'] call "+_AH2074KICKLOG+";
						};
					"; }; _A3AHstring = _A3AHstring + "
				};
			};
			...
			"; if((_MOD == 'Epoch')&&(_AWL))then{ _A3AHstring = _A3AHstring + "
				if(player == vehicle player)then
				{
					...
					{
						if(_faded)then{_faded=false;TitleText ['AntiHack by TacticalGaming.net','PLAIN DOWN'];};
					}
					else
					{
						{
		...
		_log = 'Loop #0 ended!';
		[profileName,getPlayerUID player,'BAN',toArray(_log)] call "+_AH2074KICKLOG+";
		[] spawn "+_AHKickOFF+";
	};
	[_name,_puid,_admins] spawn {
		_name = _this select 0;
									...
									{
										{
											if!((toLower _var) find _x isEqualTo -1)then
											{
												...
												[profileName,getPlayerUID player,'HLOG_SKICK',toArray(_log)] call "+_AH2074KICKLOG+";
												[] spawn "+_AHKickOFF+";
												...
												"; if(_UBV2C)then{ _A3AHstring = _A3AHstring + "
													...
													if(!isNil '_m')then
													{
														...
														if((toLower _mstring) find _x != -1)then
														{
															...
															[profileName,getPlayerUID player,'SLOG',toArray(_log)] call "+_AH2074KICKLOG+";
															(findDisplay 46)closeDisplay 0;
															missionNamespace setVariable[_var,nil];
														};
													};
		...
		[_name,_puid] spawn {
			...
			_FiredCheck = {
				...
				if(("+str _UFA+") && {((!("+str _UAW+") && {_ammo in "+str _ForbiddenAmmo+"}) || (("+str _UAW+") && {!(_ammo in "+str _AmmoWhiteList+")}))})then
				{
					...
					[profileName,getPlayerUID _unit,'BAN',toArray(_log)] call "+_AH2074KICKLOG+";
					[] spawn "+_AHKickOFF+";
				};
								...
								if(_maxAmmo > 1)then
								{
									...
									if(diag_tickTime - "+_lastshotmade+" > 0.2)then
									{
										...
										if((_camm >= _maxAmmo)&&(_muzzle isEqualTo currentWeapon _unit))then
										{
											...
											[profileName,getPlayerUID _unit,'SLOG',toArray(_log)] call "+_AH2074KICKLOG+";
				...
				if((!isNull (findDisplay 12))&&(visiblemap))then
				{
					...
					if(!_loggedOnce)then
					{
						...
						if(_size > 5)then
						{
							...
							[_name,_puid,'SLOG',toArray(_log)] call "+_AH2074KICKLOG+";
							_loggedOnce = true;
							lbClear _control2;
						};
					};
				...
				
				{
					if(!isNull _x)then
					{
						...
						else
						{
							...
							{
								systemChat format['<TacticalGaming.net> %1 has been closed.',_strx];
								_x closeDisplay 0;
								closeDialog 0;closeDialog 0;closeDialog 0;
								_wasclosed = true;
							}
						...
						if((isNull _driver)||{(player isEqualTo _driver)})then
						{
							if(!isNil '"+_AHpos+"')then
							{
								...
								if(_tmpAHpos isEqualTo [])then
								{
									...
									if(_tpcount > 3)then
									{
										[_name,_puid,'TMPBAN',toArray(_log)] call "+_AH2074KICKLOG+";
										[] spawn "+_AHKickOFF+";
									}
									else
									{
								...
								else
								{
									...
									[_name,_puid,'TPLOG',toArray(_log),_tmpAHpos select 0,_tmpAHpos select 1,_lastpos,_curpos] call "+_AH2074KICKLOG+";
								};
							}
							else
							{
...
"; }; _A3AHstring = _A3AHstring + "
		...
		[_name,_puid,_admins] spawn {
					...
					[] spawn {
						...
						if(time - "+_clickOnMapTimer+" > 5)then
						{
							...
							if("+_clickOnMapCaught+" >= 3)then
							{
								...
								[profileName,getPlayerUID player,''SLOG_SKICK'',toArray(_log)] call "+_AH2074KICKLOG+";
							};
						};
					};
				';
			...
			while{1==1}do
			{
				...
				if(!isNull player)then
				{
					...
					"; if(_LVC)then{ _A3AHstring = _A3AHstring + "
						...
						if(!(player isEqualTo _veh) && !(_vehptype isEqualTo (typeOf _veh)))then
						{
							...
							{
								[_name,_puid,'LVC',player,_vehptype,mapGridPosition _veh] call "+_AH2074KICKLOG+";
							};
						};
					"; }; _A3AHstring = _A3AHstring + "
				};
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					...
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Money Hack: [%1] should be [%2]',life_cash,_rnd1])] call "+_AH2074KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd2 isEqualTo life_adminlevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Adminlevel Hack: [%1] should be [%2]',life_adminlevel,_rnd2])] call "+_AH2074KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd3 isEqualTo life_coplevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Coplevel Hack: [%1] should be [%2]',life_coplevel,_rnd3])] call "+_AH2074KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd4 isEqualTo life_fnc_MPexec)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_fnc_MPexec [%1] should be [%2]',life_fnc_MPexec,_rnd4])] call "+_AH2074KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd5 isEqualTo life_atmbank)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_atmbank [%1] should be [%2]',life_atmbank,_rnd5])] call "+_AH2074KICKLOG+";[] spawn "+_AHKickOFF+";
					};
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '2074';
	_devLog = format['<TacticalGaming.net> %1 VERSION: 01-Dec-2016 19-28-44 TacticalGaming.net AHAT (v0244a) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
	systemChat format['%1 <TacticalGaming.net> Successfully Loaded In.',time];
	if(_puid in "+str _devs+")then{diag_log str _admins;{diag_log format['<TacticalGaming.net> %1',_x];} forEach diag_activeSQFScripts;};
	...
	if(_puid in _admins)then
	{
		...
		if(_puid in "+_adminsA+")then
		{
			systemChat format['%1 <TacticalGaming.net> Welcome Admin!',time];
...
_from = 'nelson16852@gmail.com';
_vers = '01-Dec-2016 19-28-44 - v0244a';
...
diag_log format['<TacticalGaming.net> %1 - Thread MAIN: none-threaded code compiled and/or sent!',time];
[] spawn {
	diag_log format['<TacticalGaming.net> %1 - Thread #1: Server Loop #1 starting now!',time];
	"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
		"; if(_CLG)then{ _A3AHstring = _A3AHstring + "
		...
		"; }; _A3AHstring = _A3AHstring + "
	"; }; _A3AHstring = _A3AHstring + "
	diag_log format['<TacticalGaming.net> %1 - Thread #1: Server Loop #1 looping now!',time];
	_admins = "+str _admins+";
			...
			{
				{
					...
					{
						_name = name _x;
						[_name,_puid,'BAN',toArray('')] call "+_FNC_AH2074_KICKLOG+";
					};
				} forEach playableUnits;
	...
	"; if((_MOD == 'Epoch')&&(_MPH))then{ _A3AHstring = _A3AHstring + "
		LOG_MPHIT_HANDLER =
		{
			...
			if(_log!='')then
			{
				'ARMA_LOG' callExtension format['A3_MPHIT:%1 (v0244a)',_log];
			};
		};
...
_DO_THIS_MORE_OFTEN = {
	...
	[] spawn {uiSleep 1;if(isNil '""+_antiantihack_rndvar+""')then{diag_log '<TacticalGaming.net> kicked to lobby #9';(findDisplay 46)closeDisplay 0;};};
	...
	if(hasInterFace)then
	{
		...
		} forEach ['"+_AH2074_KICKLOG+"','"+_adminsA+"','"+_MC+"'];
		{
			if(!isNil _x)then
			{
				_var = missionNamespace getVariable _x;
		...
		};
		{
			...
			if(!isNil '_pNs')then
			{
				...
				[profileName,getPlayerUID player,'HLOG',toArray(_log)] call "+_AH2074KICKLOG+";
				profileNamespace setVariable[_x,nil];saveprofileNamespace;
			};
		...
		while{1==1}do
		{
			...
			{
				_timer0 = time + 20;
				'"+_AH2074_KICKLOG+"' addPublicVariableEventHandler {(_this select 1) call "+_FNC_AH2074_KICKLOGSPAWN+";};
				uiSleep 1;
											...
											if!(_uid in "+_tokenSentArray+")then
											{
												if(_uid in "+_connectedArray+")then
												{
													...
													if(_timePassed > 60)then
													{
														...
														[_name,_uid,'HLOG_SKICK',toArray(_log)] call "+_FNC_AH2074_KICKLOG+";
													};
												};
											};
										"; }; _A3AHstring = _A3AHstring + "
...
_zeroCode = {
	if(hasInterface)then
	{
		_AHKickOFF = compileFinal '
			sleep 0.1;
			(findDisplay 46)closeDisplay 0;
			if(!isNil''"+_AH2074KICKLOG+"'')then{[profileName,getPlayerUID player,''KICKME''] call "+_AH2074KICKLOG+";};
		';
		if(isNil'"+_AHKickOFF+"')then{"+_AHKickOFF+" = _AHKickOFF;};
		if(str _AHKickOFF != str "+_AHKickOFF+")then{[] spawn _AHKickOFF;};
		_AH2074KICKLOG = compileFinal ""
			if(!isNil'"+_YourPlayerToken+"')then
			{
				"+_AH2074_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AH2074_KICKLOG+"';"+_AH2074_KICKLOG+"=nil;
			}
			else
			{
				_this spawn {
					...
					if(isNil'"+_YourPlayerToken+"')exitWith{KICKED_FOR_NIL_TOKEN_AH2074KICKLOG = format['PLAYER__%1__%2',getPlayerUID player,_this];publicVariableServer 'KICKED_FOR_NIL_TOKEN_AH2074KICKLOG';(findDisplay 46)closeDisplay 0;};
					"+_AH2074_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AH2074_KICKLOG+"';"+_AH2074_KICKLOG+"=nil;
				};
			};
		"";
		if(isNil'"+_AH2074KICKLOG+"')then{"+_AH2074KICKLOG+" = _AH2074KICKLOG;};
		if(str _AH2074KICKLOG != str "+_AH2074KICKLOG+")then{[] spawn _AHKickOFF;};
...
_b = {[] spawn {sleep 20;waitUntil {time > 20};diag_log "<TacticalGaming.net> license is blocked.";diag_log "<TacticalGaming.net> if you did not do anything wrong. please open a support ticket on http://support.infiSTAR.de";diag_log "<TacticalGaming.net> infiSTAR will NOT START when the license is blocked!";[] spawn {sleep 1;failMission "LOSER";endMission "LOSER";forceEnd;};if(!isNil"FN_GET_SERVERPW")then{(call FN_GET_SERVERPW) serverCommand "#shutdown";};};};
...
{
	[] spawn {
		diag_log format['<TacticalGaming.net> %1 - Thread #2: starting now! - SERVER FPS: %2 - activeSQFScripts: %3',time,diag_fps,count diag_activeSQFScripts];
		waitUntil {time > 120};
		diag_log format['<TacticalGaming.net> %1 - Thread #2: waited 120s - SERVER FPS: %2 - activeSQFScripts: %3',time,diag_fps,count diag_activeSQFScripts];
		setTerrainGrid 50;
		...
		diag_log format['<TacticalGaming.net> %1 - Thread #2: SCRIPT DONE - SERVER FPS: %2 - activeSQFScripts: %3',time,diag_fps,count diag_activeSQFScripts];
	};
};
...
{
	[] spawn {
		diag_log format['<TacticalGaming.net> %1 - Thread #3: starting now! - SERVER FPS: %2 - activeSQFScripts: %3',time,diag_fps,count diag_activeSQFScripts];
		waitUntil {time > 300};
		_FUNCTION_QUESTAR = profileNamespace getVariable 'FUNCTION_QUESTAR';
		if(!isNil '_FUNCTION_QUESTAR')then
		{
		...
		diag_log format['<TacticalGaming.net> %1 - Thread #3: TERMINATED - SERVER FPS: %2 - activeSQFScripts: %3',time,diag_fps,count diag_activeSQFScripts];
	};
};
call compile ("
	FNC_DO_CLIENT_VAR_DUMP = compileFinal ""
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- CURRENTLY DUMPING - WAIT UNTIL IT IS DONE (max 120s) --- (v0244a)',_name,_puid];
			diag_log format['<TacticalGaming.net>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- CURRENTLY DUMPING - WAIT UNTIL IT IS DONE (max 120s) --- (v0244a)',_name,_puid];
		};
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2: Need at least 1 more connected & alive player to dump vars of %1(%2)! (v0244a)',_name,_puid];
			diag_log format['<TacticalGaming.net>A3_CLIENT_VAR_DUMP_%1_%2| Need at least 1 more connected & alive player to dump vars of %1(%2)! (v0244a)',_name,_puid];
		};
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- ALREADY DUMPING THIS PLAYER --- (v0244a)',_name,_puid];
			diag_log format['<TacticalGaming.net>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- ALREADY DUMPING THIS PLAYER --- (v0244a)',_name,_puid];
		};
		...
		[_name,_puid] spawn {
			...
			if(time > _timeOut)exitWith
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- DUMP TIMED OUT --- (v0244a)',_name,_puid];
				diag_log format['<TacticalGaming.net>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- DUMP TIMED OUT --- (v0244a)',_name,_puid];
			};
			...
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- ALL GOOD - NOTHING TO DUMP --- (v0244a)',_name,_puid];
				diag_log format['<TacticalGaming.net>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- ALL GOOD - NOTHING TO DUMP --- (v0244a)',_name,_puid];
			};
			
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- STARTING DUMP --- (v0244a)',_name,_puid];
			diag_log format['<TacticalGaming.net>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- STARTING DUMP --- (v0244a)',_name,_puid];
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%3',_name,_puid,_x];
				diag_log format['<TacticalGaming.net>A3_CLIENT_VAR_DUMP_%1_%2| %3',_name,_puid,_x];
			} forEach _selectedClientVars;
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- DUMP ENDED --- (v0244a)',_name,_puid];
			diag_log format['<TacticalGaming.net>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- DUMP ENDED --- (v0244a)',_name,_puid];
		};
	"";
");
if(_testserver)then{
...
diag_log format['<TacticalGaming.net> %1 - AntiHack loaded!',time];
/* ********************************************************************************* */
/* *********************************www.infiSTAR.de********************************* */
