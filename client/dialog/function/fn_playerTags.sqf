...
if (life_adminxray == 1) then {
	{
		if (isPlayer _x) then {
			 ...
			 _name = _x getVariable['life_name', name _x];
				...
				if !(!(_x isKindOf "Man") && count (crew _x) == 0) then
				{
					...
					_text = switch (true) do
					{
						case (side _x == west):
						{
							switch (_x getVariable["copLevel", 0]) do
							{
								case (1) : {format["Police Cadet %1", _x getVariable['life_name', name _x]]};
								case (2) : {format["Police Constable %1", _x getVariable['life_name', name _x]]};
								case (3) : {format["Police Corporal %1", _x getVariable['life_name', name _x]]};
								case (4) : {format["Police Sergeant %1", _x getVariable['life_name', name _x]]};
								case (5) : {format["Police Lieutenant %1", _x getVariable['life_name', name _x]]};
								case (6) : {format["Police Captain %1", _x getVariable['life_name', name _x]]};
								default { _x getVariable['life_name', _x getVariable['life_name', name _x]] };
							};
						};
						...
						default {_x getVariable['life_name', name _x]};
					};
